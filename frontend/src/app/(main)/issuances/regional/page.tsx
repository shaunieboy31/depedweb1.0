...
export default function RegionalIssuances() {
  const [searchQuery, setSearchQuery] = useState("");

  const mockDocs = [
    { id: 1, num: "RM No. 120, s. 2024", title: "REGIONAL TRAINING OF TRAINERS ON THE MATATAG CURRICULUM", date: "April 02, 2024", size: "2.4 MB" },
    { id: 2, num: "RA No. 005, s. 2024", title: "CALABARZON HEROES: RECOGNITION OF OUTSTANDING TEACHERS", date: "March 20, 2024", size: "1.2 MB" },
  ];
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 w-full space-y-12 mt-12">
           ...
           <div className="space-y-4">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-500/20 rounded-full border border-emerald-400/30 text-emerald-300 text-[10px] font-black uppercase tracking-widest">
                 <Layers size={12} />
                 <span>Regional Office IV-A</span>
              </div>
              <h1 className="text-4xl md:text-6xl font-black text-white uppercase tracking-tighter leading-none">
                 REGIONAL <span className="text-emerald-400 underline decoration-emerald-500/30 underline-offset-8">ISSUANCES</span>
              </h1>
              <p className="text-slate-400 text-sm md:text-lg font-medium max-w-2xl">
                 Official memoranda, advisories, and circulars issued by the Regional Office CALABARZON for division implementation.
              </p>
           </div>
           ...
           <div className="relative group max-w-3xl">
              ...
              <input 
                 type="text" 
                 placeholder="Search Regional Issuances..."
                 value={searchQuery}
      ...
      <div className="max-w-5xl mx-auto px-6 -mt-32 relative z-20 space-y-8">
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-4 border border-white shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4">
           <div className="flex items-center gap-4 text-slate-500">
              ...
              <div className="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-lg text-[10px] font-black uppercase tracking-widest">CALABARZON (Region IV-A)</div>
           </div>
           ...
           <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                 ...
                 <tbody className="divide-y divide-slate-100">
                    {mockDocs.map((doc) => (
                      <tr key={doc.id} className="group hover:bg-emerald-50/50 transition-all duration-300">
                         <td className="px-8 py-8 first:pl-10">
                            <span className="text-sm font-black text-slate-900 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">
                               {doc.num.split(',')[0].replace('RM No. ', '').replace('RA No. ', '')}
                            </span>
                         </td>
                         ...
                         <td className="px-8 py-8">
                            <h3 className="text-base font-black text-slate-900 group-hover:text-emerald-700 transition-colors leading-snug uppercase tracking-tight">
                               {doc.title}
                            </h3>
                         </td>
                         <td className="px-8 py-8">
                            <button className="flex items-center gap-3 px-6 py-3 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-500/10 active:scale-95">
                               <FileDown size={14} />
        ...
        <div className="bg-slate-50/50 rounded-[3rem] p-16 border-2 border-dashed border-slate-200 text-center space-y-6">
           ...
           <div className="space-y-2">
              <h4 className="text-xl font-black text-slate-900 uppercase">Regional Archive Portal</h4>
              <p className="text-slate-500 text-sm font-medium">Directives and policy announcements from the Region IV-A office.</p>
           </div>
        </div>
